...
const get_ProductItem_service_1 = require("../services/get_ProductItem_service");
...
class GetProductItemController {
    static async syncGetProductItem(req, res) {
        ...
        try {
            const products = await get_ProductItem_service_1.ApiService.fetchProductsFromExternal(token);
            ...
            for (let i = 0; i < totalRecords; i += BATCH_SIZE) {
                // ใช้ .slice เพื่อตัด Array ออกมาเป็นชุดย่อย
                const batch = products.slice(i, i + BATCH_SIZE);
                // 3. สั่ง Model ให้ Insert ข้อมูลชุดนี้
