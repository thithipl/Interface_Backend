...
export const INQ_ProformaNoModel = {
    async getDataInquiryPFMNo(): Promise<InquiryDataPFMNo[]> {
        return db<InquiryDataPFMNo>("data_2003.dbo.VW_AS_getDataInquiryPFMNo").orderBy("id", "desc").select("*");
    },
}

export const INQ_ProformaCodeModel = {
    async getDataInquiryCodePFMNO(pfmNo: string): Promise<InquiryCodePFMNO[]> {
        return db<InquiryCodePFMNO>("data_2003.dbo.VW_AS_getDataInquiryCodePFMNo").where("PFM_No", pfmNo).select("*");
    },
}