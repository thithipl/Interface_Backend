...
import { InvoiceHeader, ParsedInvoice, InvoiceItem, Claim, InvoiceShipping } from './View_invoice/m_convertJson_invoice';
/**
 * 
 * @param rawData
 */
export function createParsedInvoice(rawData: any): ParsedInvoice {
    const safeParseFloat = (value: string) => Number.parseFloat(value) || 0;
    const safeParseInt = (value: string) => Number.parseInt(value, 10) || 0;

    const convertedItems: InvoiceItem[] = (rawData.items || []).map((item: any) => ({
    ...
    const convertedClaims: Claim[] = (rawData.claims || []).map((claim: any) => ({
        claim_code: claim.claim_code || '',
    }));


    const convertedShipping: InvoiceShipping[] = (rawData.shipping || []).map((shipping: any) => ({
